...
export function registerProvenanceTools(registry: ToolRegistry<Env>) {
  ...
  registry.provenance_append = {
    ...
    execute: async (args, { env, requestId }) => {
      ...
      const key = `prov:${ts.toString().padStart(16, "0")}:${id}`;
  ...
  registry.provenance_query = {
    ...
    execute: async (args, { env }) => {
      const normalizedPrefix = (args.prefix ?? "").replace(/^prov:/, "").trim();

      const prefix = `prov:${normalizedPrefix}`;
      ...
      const items = await Promise.all(
        res.keys.map(async (k) => {
          const raw = await env.STATE_KV.get(k.name);
